...
export const fetchProductsByCategory = async (category, page = 1, limit = 100, search = '') => {
  try {
    const searchQuery = search ? `&search=${encodeURIComponent(search)}` : '';
    const res = await fetch(`${API_BASE_URL}/products/category/${category}?page=${page}&limit=${limit}${searchQuery}`, {
